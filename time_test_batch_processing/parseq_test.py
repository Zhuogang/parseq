...
def main(keyframe, parseq_path, parseq_eps=0.7, batch_size=8, device="gpu_device", img_type="jpg"):
    ...
    for i in tqdm(range(num_labels)):
        ...
        if os.path.exists(label_folder):
            text_img_paths = [os.path.join(label_folder, x) for x in os.listdir(label_folder) if x.endswith(img_type)]
            ...
            for ii in range(n_batch):
                ...
                for i_tmp in range(len(readings)):
                    ...
                    if len(reading) > 0:
                        outfilename = os.path.join(img_paths_tmp[i_tmp].replace(img_type, "txt"))
